...
func Render(before []byte, p Policy) ([]byte, []string, error) {
	...
	if p.Version == 1 {
		...
		if err := set(args, "codex", "agent_args_override.codex", []string{"--cfo-native-gate", "-c", `service_tier="default"`}); err != nil {
			return nil, nil, err
		}
		paths, err := mapping(root, "agent_path_override")
		if err != nil {
			return nil, nil, err
		}
		if err := set(paths, "codex", "agent_path_override.codex", "cfo"); err != nil {
			return nil, nil, err
		}
...
func (c Config) Apply(ctx context.Context) (result ApplyResult, err error) {
	...
	// Both copies may contain unrelated credentials. Restrict them before replacement.
	...
	if err := replaceFile(c.Path, name); err != nil {
		return result, err
	}
	return result, nil
}
