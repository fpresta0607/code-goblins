...
func writeSupervisionInstructions(ew *werr) {
	...
	ew.println("That command is REFUSED when an unanswered question sits at or below its sequence: drain names the blocked goblin and retires nothing. The refusal is the protection working, not an error - a record acked unread is a record nobody will ever read. Answer the goblin with \"cfo send <id> \\\"...\\\"\", then re-run the same command with --ack-blocking to retire the question you have now read.")
