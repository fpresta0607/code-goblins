...
// preflightSchemaJSON builds a protocol-22 schema-1 document; omit drops one
...
func fullSchemaJSON() string {
	return preflightSchemaJSON(22, 1, "", "success_response", "error_response")
}
...
func TestPreflightPinsDiscoveryArgumentSplits(t *testing.T) {
	runner := &fakeRunner{replies: []runnerReply{
		...
		rawReply(statusJSON(22, 22, true, true)),
...
func TestCheckSchemaRefusesUnsupportedContracts(t *testing.T) {
	tests := []struct {
		...
		{"unsupported schema version", preflightSchemaJSON(22, 2, "", "success_response", "error_response"), "schema version 2"},
		{"unsupported protocol", preflightSchemaJSON(20, 1, "", "success_response", "error_response"), "protocol 20"},
		{"missing success envelope", preflightSchemaJSON(22, 1, "", "error_response"), "success_response"},
		{"missing error envelope", preflightSchemaJSON(22, 1, "", "success_response"), "error_response"},
		{"missing request contract", `{"protocol":22,"schema_version":1,"schemas":{"success_response":{},"error_response":{}}}`, "request contract"},
...
func TestCheckRuntimeRefusesIncompatibleOrAmbiguousFacts(t *testing.T) {
	tests := []struct {
		...
		{"protocol mismatch", statusJSON(22, 18, true, true), sessionListJSON("fleet"), "protocol mismatch", 1},
		{"client protocol mismatch", statusJSON(18, 22, true, true), sessionListJSON("fleet"), "protocol mismatch", 1},
		{"server not running", statusJSON(22, 22, false, true), sessionListJSON("fleet"), "not running", 1},
		{"incompatible server", statusJSON(22, 22, true, false), sessionListJSON("fleet"), "compatibility", 1},
		{"malformed status", `{`, sessionListJSON("fleet"), "decode status", 1},
		{"session not addressable", statusJSON(22, 22, true, true), sessionListJSON("other"), "not addressable", 2},
		{"ambiguous session", statusJSON(22, 22, true, true), sessionListJSON("fleet", "fleet"), "ambiguous", 2},
		{"malformed session list", statusJSON(22, 22, true, true), `{`, "decode session list", 2},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
