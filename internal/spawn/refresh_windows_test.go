...
func TestAuthRefreshSkipsAnUnreadableTaskRecordAndRefreshesTheRest(t *testing.T) {
	...
	refresher := AuthRefresher{StateDir: stateDir, Store: store, Panes: stubPanes{live: map[string]bool{"pane-live": true, "pane-stuck": true}}, Warn: &warn}
	refreshed, err := refresher.RefreshProject(context.Background(), project)
	if err != nil {
		t.Fatalf("RefreshProject aborted on an unreadable record: %v", err)
	}
	...
	if !strings.Contains(warn.String(), "stuck-1") {
		t.Errorf("warnings = %q, want the skipped record named", warn.String())
	}
}
