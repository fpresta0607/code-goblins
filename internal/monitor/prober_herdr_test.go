...
func proberSnapshotEnvelope(metas []state.TaskMeta, statuses map[string]string) string {
	var b strings.Builder
	b.WriteString(`{"id":"cli:api:snapshot","result":{"type":"session_snapshot","snapshot":{"version":"0.8.0-test","protocol":22,`)
...
func TestHerdrProberStructuralVerdicts(t *testing.T) {
	...
	envelope := func(tabs, panes, agents string) string {
		return `{"result":{"type":"session_snapshot","snapshot":{"protocol":22,"workspaces":[{"workspace_id":"ws","label":"cfo"}],"tabs":[` + tabs + `],"panes":[` + panes + `],"agents":[` + agents + `]}}}`
	}
