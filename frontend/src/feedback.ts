...
export function deliveryMark(action: Action): { icon: IconName; label: string; trouble: boolean } {
  ...
  switch (action.status) {
    case "succeeded":
      ...
      return { icon: "check-double", label: cfo ? "Accepted by the CFO" : action.kind === "goblin_answer" ? "Delivered to the goblin" : action.kind === "review_answer" ? "Delivered" : "Done", trouble: false };
