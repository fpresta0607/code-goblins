...
export function GoblinPanel({ task, node, snapshot, connected, reviews, view, onView, onOwner, leading, trailing }: {
  task?: Task; node?: Session; snapshot: Snapshot; connected: boolean; reviews: ReviewControls;
  view: PanelView; onView: (view: PanelView) => void; onOwner?: () => void; leading?: ReactNode; trailing: ReactNode;
}) {
  ...
  return <section className="goblin-panel" aria-labelledby="panel-title">
    ...
    <PanelHeader task={task} node={node} snapshot={snapshot} compact={view === "terminal"} />
