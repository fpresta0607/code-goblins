        ...
        <fieldset disabled={!pending || draft?.sending}><legend className="sr-only">Choose your answer</legend>
          {questionChoices(question).map((option) => <label className="question-choice" key={option.value}>
            ...
            <span className="question-option"><span>{option.label}. {option.value}</span>{option.recommended && <span className="recommendation">Recommended</span>}
              {option.image && <span className="question-image"><img src={option.image} alt={"Option " + option.label + ": " + option.value} /><a className="icon-button" href={option.image} target="_blank" rel="noreferrer" aria-label={"Open option " + option.label + " full size"} title="Open full size"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 4h6v6M20 4l-8 8M10 5H5v14h14v-5" /></svg></a></span>}</span>
          </label>)}
