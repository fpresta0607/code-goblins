...
export function Questions({ snapshot, connected, presentations }: { snapshot: Snapshot; connected: boolean; presentations:BoardActivity[] }) {
  ...
  return <>
    <details className="command-center-menu"><summary>Command Center{waiting > 0 && " (" + waiting + ")"}</summary><div className="command-center-updates">
      ...
      {questions.length > 0 && <button onClick={() => setSelected(questions.find(q=>q.status==="pending")?.id || questions[questions.length-1].id)}>{waiting ? "Answer CFO question" : "View last decision"}</button>}
    ...
    <dialog ref={dialog} className="question-modal" aria-labelledby="command-center-heading" aria-describedby="question-heading" onCancel={(event) => { event.preventDefault(); dismiss(); }} onKeyDown={(event) => event.stopPropagation()}>
      {question && <form onSubmit={(event) => { event.preventDefault(); void send(); }}>
        ...
        <header className="command-center-heading"><Avatar persona="cfo" /><div><h2 id="command-center-heading">Supreme Overlord<span>Command Center</span></h2><p><span className="status-dot" />{pending ? "CFO needs your decision" : question.answer_id || outcome ? "Your decision" : "Question closed"}</p></div></header>
        ...
        <fieldset disabled={!pending || draft?.sending}><legend className="sr-only">Choose your answer</legend>
          ...
          {displayed.selection === "other" && <label className="written-answer"><span className="sr-only">Your written answer</span><textarea rows={3} maxLength={4000} placeholder="Tell the CFO what you prefer..." value={displayed.written} onChange={(event) => update(question.id, { written: event.target.value, error: "", receipt: undefined })} /></label>}
        </fieldset>
