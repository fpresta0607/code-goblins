...
export function RunCard({ run, connected, sending, error, onRun }: { run: Run; connected: boolean; sending: boolean; error: string; onRun: () => void }) {
  ...
  return <article className="run-card" aria-labelledby={"run-" + run.id}>
    ...
    {run.state === "ready" && <div className="card-actions">
