...
import { CFO_KEY, keepLive, paneWidth, switchKey, switchOrder, switchTarget } from "./terminalOrder.ts";
import type { Task } from "./types.ts";

const task = (id: string, changes: Partial<Task> = {}) => ({ id, generation: "g1", archived: false, ...changes }) as Task;
const key = (code: string, changes: Partial<{ ctrlKey: boolean; altKey: boolean; shiftKey: boolean; metaKey: boolean }> = {}) => ({ code, ctrlKey: true, altKey: true, shiftKey: false, metaKey: false, ...changes });
...
test("Ctrl+Alt with an arrow cycles and with a digit jumps; nothing else is taken from the terminal", () => {
  ...
  for (const other of [key("Digit0"), key("KeyA"), key("ArrowDown", { shiftKey: true }), key("ArrowDown", { altKey: false }), key("Digit2", { ctrlKey: false }), key("Digit2", { metaKey: true })]) assert.equal(switchKey(other), null, JSON.stringify(other));
});
...
test("the divider keeps both the board and the panel usable", () => {
  ...
  assert.equal(paneWidth(1500, 1600), 1320);
