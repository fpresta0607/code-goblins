...
description: >-
  Agent-only reference for changing CFO's shared, tracked material per AGENTS.md section 1.
  Use before editing any of that material, whether working as CFO directly or as a goblin briefed on a code-goblins-repo task.
...
Load this before changing CFO's shared, tracked material, as defined by `AGENTS.md` section 1.
...
1. Does the CFO AGENT need this on every session or every turn to operate?
...
Briefs for tasks that touch CFO's own tracked material should tell the goblin to load this skill.
`bin/fm-brief.sh`'s `REPO` argument is a caller-supplied string with no reliable signal that it names CFO's own repo, unlike a project registered in `data/projects.md`, so there is no clean point inside the scaffold to detect this case automatically.
CFO adds this skill's load instruction to code-goblins-repo briefs by hand instead.
