---
name: firstmate-orca
description: Agent-only operator checklist for CFO's Orca runtime backend. Use when switching to Orca, spawning or supervising Orca-backed work, smoke-testing Orca backend behavior, debugging Orca task state, or reconciling Orca-backed task metadata.
...
Use this as the operator checklist for CFO's experimental Orca runtime backend.
...
Use raw `orca` only when the helper surface cannot answer the inspection question, and keep the recorded CFO metadata as the task identity.

## Preflight

Work from the current CFO home or repo root.
...
Use `bin/fm-spawn.sh` so CFO creates the brief, worktree, terminal, metadata, status file, and watcher surface together.
...
After spawn, check the task with CFO helpers:
...
Do not manually create the Orca worktree or terminal for a normal CFO task.
Do not manually patch metadata to make an externally-created Orca terminal look like a CFO task.
...
Put long instructions in the task brief or a temporary file and point the goblin at that file.

When supervising, treat `state/<id>.meta` as the routing record and Orca's own ids as backend implementation details.
The stable CFO alias is `fm-<id>`.
...
4. Prefer CFO helpers for peek, send, state, and teardown.
...
Teardown remains governed by the normal CFO landing rules.
