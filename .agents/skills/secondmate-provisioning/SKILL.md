...
Keep the always-inline routing rules in `AGENTS.md` authoritative: route by natural-language `scope:`, local-only projects stay with the main CFO, and secondmates are idle by default.
...
For a remote route, `host:` is an OpenSSH config alias and `root:` is that host's separate tracked CFO code root.
...
Pass `--no-projects` instead of a project list to scaffold a project-less charter for a domain whose subject is the CFO repo itself, whose home is a CFO worktree and whose crews take pooled worktrees of the same repo.
...
You resolve each project's origin yourself - from the Supreme Overlord, the project registry, a clone that exists elsewhere, `gh-axi`, or an explicit paste - and name it as `<project>=<origin-url>`; the seed validates and transports what you supply.
...
`-` durably leases a fresh CFO worktree via `treehouse get --lease` under the secondmate id.
...
This is secondmate-only: goblin/scout model resolution is untouched by this file.
...
Before a local launch, `fm-spawn.sh --secondmate` locally fast-forwards the home to the primary CFO checkout's current default-branch commit when it is safe; dirty, diverged, or in-flight homes launch unchanged with a warning.
...
Inheritance copies the literal `config/crew-harness` file, so a secondmate's own goblins use the primary's goblin harness only when it names a concrete adapter such as `codex`; an unset or `default` value has nothing concrete to inherit, and the secondmate's own goblins fall back to the secondmate's own or detected harness instead.
...
`data/overlord-shared.md` is main-authoritative in the primary home and read-only in secondmate homes.
Its primary file header must state that the file is main-authoritative, read-only in secondmate homes, must not be edited there, and that new overlord-preference discoveries are routed to the main CFO through marked status or a document pointer.
...
Never copy any secondmate `data/overlord-shared.md` back into the primary.
Keep each home's `data/overlord.md` domain-local.
After first propagation to an existing home, trim that home's local `data/overlord.md` by hand to domain-specific content plus pointers to `data/overlord-shared.md`; do not automate or silently delete private content.
Keep every `data/learnings.md` fully local by Supreme Overlord decision; route fleet-general machinery facts into tracked documentation through the normal CFO repo path rather than inventing shared learnings propagation.
...
`data/overlord-shared.md` is not a config file and is never inlined into this instruction file or message.
...
`local-only` projects stay with the main CFO.
...
Scope-matching is CFO's judgment against the secondmate's natural-language scope, not a keyword rule.
...
This delegated route remains required when `config/backlog-backend=manual`, which controls only routine CFO backlog edits.
...
It refuses any destination that is not a genuine seeded CFO home with safe operational directories and a matching `.fm-secondmate-home` marker, so a move can never land in a project.
...
The main CFO reconciles only direct reports.
Each secondmate is a CFO in its own home, so it runs recovery on startup and reconciles its own goblins.
...
Run `bin/fm-teardown.sh <id>` for `kind=secondmate` only when the Supreme Overlord or main CFO explicitly decides to retire that persistent second mate.
...
Never use `--force` unless the Supreme Overlord explicitly said to discard the work.
