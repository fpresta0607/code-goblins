...
description: >-
  Agent-only playbook for coordinating visible Codex Desktop threads alongside CFO without pretending they are a selectable shell backend.
  Use before creating, reading, steering, archiving, debugging, or reviewing a Codex App visible thread for CFO work, and before responding to requests to make Codex App native to CFO.
...
Use this playbook when CFO work needs a visible Codex Desktop thread.
...
Codex Desktop visible threads are companion host-tool workflows, not a selectable CFO backend.
...
If local helper scripts exist for Codex App work, use only helpers explicitly provided by the operator or maintained by CFO.
...
3. Do not create projectless threads for repo work.
   If the project is absent, stop and ask for the project to be added or use a normal CFO backend instead.
4. Decide whether this is a real CFO-managed task or a visible companion thread.
...
A Desktop-owned Codex thread can append to CFO status files only when the prompt gives an absolute path and the Desktop permission context can write that checkout.
That makes status writes a verified return-channel requirement, not a fact to assume.

For a CFO-managed task, include an explicit status instruction:

```text
Append supervisor-visible status lines to <absolute-CFO-home>/state/<task-id>.status.
Use only these prefixes for status changes: working:, needs-decision:, blocked:, paused:, done:, failed:.
Use paused: only for a deliberate known external wait that should be rechecked later, never for a blocker that needs CFO to act.
...
Do not claim it is a complete CFO backend.
...
For CFO reconciliation, prefer concrete evidence:
...
Avoid repeating long transcripts into CFO docs or PR bodies.
Summarize only the host-tool calls, the status-file result, and the archive result.
When reporting a Desktop-thread result to the Supreme Overlord, translate status prefixes and return-channel evidence through `AGENTS.md` section 9.
...
If there is a real CFO task record, leave teardown decisions to the normal CFO task flow instead of this skill.
