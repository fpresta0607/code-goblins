...
description: Agent-only reference for CFO harness operations. Use before spawning or recovering a goblin or secondmate, handling a trust dialog, sending a harness-specific skill invocation, interrupting or exiting an agent, resuming an exited agent, or verifying a new harness adapter. Contains verified facts for claude, codex, opencode, pi, pi-signed, grok, kimi, and muse.
...
Use this reference before any harness-specific CFO operation: spawn, recovery, trust-dialog handling, skill invocation, interrupt, exit, resume, or adapter verification.

Goblins default to the same harness CFO is running on unless `config/crew-harness` records an adapter name.
Optional dispatch profiles in `config/crew-dispatch.json` can override that static default for one goblin or scout dispatch by selecting concrete harness, model, and effort axes at intake.
...
The Supreme Overlord may override that file at session start or later; a per-task instruction such as "run this one on codex" overrides it for that dispatch only.
`default` means mirror CFO's own harness.

Secondmates have their own harness knob, so a secondmate can run on a different adapter than goblins.
`config/secondmate-harness` is the harness the primary uses to launch SECONDMATE agents, resolved through the fallback chain `config/secondmate-harness` -> `config/crew-harness` -> CFO's own.
...
This skill owns only the harness-relevant consequence: a secondmate's own goblins use the primary's inherited dispatch profiles and static harness value, while `config/secondmate-harness` is the primary's own setting and is never inherited - secondmates do not spawn secondmates.
Inheritance copies the literal `config/crew-harness` file, so for a secondmate's own goblins to run on the primary's goblin harness the Supreme Overlord must set `config/crew-harness` to a concrete adapter name, such as `codex`.
If `config/crew-harness` is unset or `default`, there is no concrete value to inherit, so the secondmate's own goblins fall back to the secondmate's own/detected harness rather than the primary's effective goblin harness.
Inheritance also copies the literal `config/crew-dispatch.json` file, so secondmates apply the same best-fit profile rules for their own goblins.

Each adapter splits into mechanics and knowledge.
The per-task mechanics, including launch command, autonomy flag, and any enabled goblin turn-end hook, live in `bin/fm-spawn.sh`.
...
The per-adapter `Exit command` and `Interrupt` rows below remain the verification record for those values; the executable owner is what CFO actually runs, so a newly verified adapter is not reachable by the control plane until its rows land in that owner.
...
`bin/fm-harness.sh crew` resolves the effective goblin harness from `config/crew-harness` (absent or `default` -> own).
...
`bin/fm-spawn.sh` uses `crew` mode for a goblin/scout launch and `secondmate` mode for a `--secondmate` launch, re-resolving on every spawn so the split is durable across respawns; an explicit per-spawn harness arg overrides either.
On `unknown`, ask the Supreme Overlord instead of guessing.
A Supreme Overlord override always beats detection.
...
muse is goblin/SCOUT ONLY and has no primary integration at all: its plugin engine (its only hook surface) is disabled in the default build, and its Claude-compatible hook dialect names `asyncRewake` and model reawakening as explicitly unsupported, which is exactly what a CFO primary's turn-end supervision needs.
...
`bin/fm-spawn.sh` accepts concrete `--harness`, `--model`, and `--effort` values chosen by CFO at intake.
Do not make the shell scripts parse or match natural-language dispatch rules.

Effort precedence is an explicit per-task Supreme Overlord instruction first, then any applicable standing dispatch profile or secondmate pin, then the generic fallback below.
Never replace an effort value supplied by either higher-precedence source.
Use the fallback only when neither the Supreme Overlord nor applicable standing configuration specifies effort.
...
Never select `max` from this fallback; use it only when the Supreme Overlord has explicitly expressed that per-task or standing preference.
...
| grok | `--model <model>` | `--reasoning-effort <low\|medium\|high>` | Verified on grok 0.2.99 (2026-07-13). `--effort` is an alias, but CFO's profile axis is reasoning effort. As of 0.2.99 the ceiling is `high`; both `xhigh` and `max` are rejected with `use one of: high, medium, low`, so CFO omits them. |
| pi / pi-signed | `--model <model>` | `--thinking <low\|medium\|high\|xhigh\|max>` | Verified 2026-07-27 on Pi and pi-signed 0.82.0. Both expose the same accepted thinking levels and completed the same model-qualified max-thinking smoke. |
| opencode | `--model <provider/model>` | none for CFO's interactive launch | Verified on opencode 1.17.6. `opencode run` has `--variant`, but CFO launches the interactive `opencode --prompt` path, which has no verified effort flag. |
| kimi | `--model <model>` | none | Verified 2026-07-25 on Kimi Code CLI 0.29.1. |
| muse | `--model <model>` | `--reasoning-effort <low\|medium\|high\|xhigh>`, and `ultra` only for an explicit `max` | Verified 2026-08-05 on Muse Code 0.1.0-R708.1. The flag accepts `none\|minimal\|low\|medium\|high\|xhigh\|ultra` and defaults to `high`. `ultra` is muse's max-class level, so it is reachable only through an explicit Supreme Overlord `max`, never from the generic fallback; `none` and `minimal` sit below the shared vocabulary and stay unreachable. |
...
If such a dialog is showing, accept it from an active CFO session using `FM_HOME=<this-firstmate-home> bin/fm-send.sh <window> --key Enter`, or the choice the dialog requires, unless `FM_HOME` is already set to the active CFO home; verify the brief started processing.

Claude renders a predicted-next-prompt suggestion as dim/faint text inside an otherwise-empty composer after a turn completes.
A plain `tmux capture-pane` cannot tell that ghost text apart from typed text.
CFO launches every claude goblin and secondmate with `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false`, scoped to CFO-launched agents through `bin/fm-spawn.sh`, so it never touches the Supreme Overlord's global config.
...
As defense in depth for any pane that flag cannot reach, including the Supreme Overlord's own CFO composer that away-mode reads, the shared `fm_composer_strip_ghost` extractor in `bin/fm-composer-lib.sh` removes dim/faint SGR 2 ghost runs before pending-input classification on every styled reader (tmux, herdr, and Zellij).
...
This is separate from the per-task goblin turn-end hook above (that one just `touch`es a marker file in a task's own `.claude/settings.local.json`).
The CFO PRIMARY's own `.claude/settings.json` registers two Stop hooks: `bin/fm-turnend-guard.sh --claude` and the Stop-owned auto-arm `bin/fm-claude-stop-autoarm.sh` (`asyncRewake: true`, `timeout: 28800`), and exiting the guard with status 2 plus stderr reliably forces the model to continue.
...
A project-level `.claude/settings.json` only takes effect when Claude Code's project root is that exact directory - it does not walk up from a subdirectory looking for one, so CFO launches the primary from the repo root.
...
| Fact | Value |
|---|---|
| Busy state | Unknown until a semantic source is live-verified: the app-server turn lifecycle is unreachable for a pane worker, and project lifecycle hooks did not fire for a CFO-launched worker. |
...
The CFO PRIMARY's own `.codex/hooks.json` registers a Stop hook that pipes Codex's Stop payload to `bin/fm-turnend-guard.sh`.
...
The tracked hook anchors to `pwd -P`, verifies that root is CFO-shaped and hook-bearing, and then invokes `bin/fm-turnend-guard.sh` with the original payload.
...
| Fact | Value |
|---|---|
| Busy state | The CFO-owned plugin's semantic `session.status`: `busy` and `retry` are active, `idle` is inactive, latched to the worker's own session. |
| Exit command | `/exit` |
...
The CFO PRIMARY's own `.opencode/plugins/fm-primary-turnend-guard.js` listens for `session.idle`.
...
| Fact | Value |
|---|---|
| Busy state | The CFO-owned extension's `agent_start` (busy) and `agent_settled` confirmed by `ctx.isIdle()` (idle), which covers retries, compaction, tool loops, and queued continuations. |
| Exit command | `/quit` |
| Interrupt | single Escape |

Pi has no permission system, so goblins are always autonomous.
Pi's `packages/coding-agent/docs/settings.md` UI and display section documents `regular` as the `tuiMode` default and `fullscreen` as experimental; fullscreen can bury steers by rewriting scrollback, so CFO avoids it when the installed CLI supports the override.
...
CFO records `pi-signed` without normalization and refuses rather than falling back to `pi` when that wrapper is unavailable.
...
CFO sets `FM_PI_HARNESS` explicitly for both worker launch identities, and a signed primary uses the README launch command to establish the same boundary.
...
The CFO PRIMARY's own `.pi/extensions/fm-primary-turnend-guard.ts` listens for logical-run `agent_settled`, not per-tool-loop `turn_end`, and uses `pi.sendUserMessage(..., { deliverAs: "followUp" })` to force one guarded follow-up when `bin/fm-turnend-guard.sh` returns 2.
...
**Incident (2026-07-03, herdr backend only, grok 0.2.82):** two grok/herdr goblins were sent `/no-mistakes` via `fm-send`; both left it fully typed but unsubmitted in the composer for minutes (footer still `Enter:send`), and `fm-send` exited 0 with no error.
...
Turn-end hook: grok fires a `Stop` hook at every turn boundary, giving CFO a precise per-turn wake instead of only stale-pane detection.
grok loads PROJECT hooks (`<worktree>/.grok/hooks/`, `<worktree>/.claude/settings.local.json`) only after the folder is granted hook-trust in `~/.grok/trusted_folders.toml`, which is not automatic and which CFO will not establish by editing grok's own managed trust store.
GLOBAL hooks in `~/.grok/hooks/` are always trusted and load on first launch.
So `fm-spawn` installs ONE CFO-owned global hook, `~/.grok/hooks/fm-turn-end.json`, plus the companion `~/.grok/hooks/fm-turn-end.sh`, guarded as a no-op for every non-CFO grok session.
Its `Stop` command fires only when the current workspace holds a `.fm-grok-turnend` token pointer that matches the CFO-owned hook registry under `~/.grok/hooks/fm-turn-end.d/`.
...
This keeps the hook outside the worktree, needs no trust grant, and writes only CFO-owned files.
...
The CFO PRIMARY's own `.grok/hooks/fm-primary-turnend-guard.json` invokes `bin/fm-turnend-guard-grok.sh`.
...
Project-local Grok hooks require folder trust, verified with launch-time `--trust`; if the primary CFO checkout is not trusted for Grok hooks, this primary guard fails open and `fm-guard.sh` remains the next-command alarm.
...
| Skill invocation | `/<skill>`, for example `/no-mistakes`; CFO skills are discovered. |
...
[`docs/turnend-guard.md`](../../../docs/turnend-guard.md) owns Kimi's verified global hook surface and overlord-approved crew wake integration.
`fm-spawn.sh` installs one marker-delimited CFO entry in `$HOME/.kimi-code/config.toml`, one silent always-zero hook script, and one private token registry under `$HOME/.kimi-code/fm-turn-end.d/`.
...
Muse Code is a goblin and SCOUT adapter only.
`bin/fm-spawn.sh` refuses `--secondmate` on muse, and muse has no supervision protocol under `docs/supervision-protocols/`, so a CFO primary detected as muse falls back to the `unknown` protocol.
...
| Trust dialog | `Do you trust this workspace?` with `1 Trust and continue` preselected, accepted by Enter. `--yolo` suppresses it entirely, which is what CFO relies on because every task gets a fresh worktree path. |
...
Escalate that refusal to the Supreme Overlord as a needed credential.
...
It was verified to drop the foreign `rules_file` context block while KEEPING a project's own `AGENTS.md` rules, which the goblin contract depends on.
...
Never use `--no-session-log` for a goblin: it disables the only busy source muse has.
...
CFO deliberately does NOT exclude any muse path from `fm-teardown.sh`'s uncommitted-work check.
CFO writes `.claude/settings.local.json` itself, which is why that path is excluded for claude; it does not write muse's, so a nested muse worktree or leftover scratch is the agent's own work product and MUST be able to refuse teardown.
...
The Supreme Overlord accepted that risk, so CFO does NOT set `MUSE_NO_AUTO_UPDATE=1`; a fleet that later wants stability can set it in the launch environment without any adapter change.
