...
description: >-
  Agent-only procedure for CFO project management.
...
Cloning or registering a project is add intake and uses the same trigger.
This skill is the single owner of CFO's project-management procedure.
...
Keep each registry description useful for identifying the project, but keep delivery posture, overlord-private state, and detailed project knowledge in their existing designated homes.
...
If the owning second mate cannot accept the route, report that concrete blocker or obtain an explicit Supreme Overlord redirection rather than silently duplicating the project in the main home.
...
The registry records the project's standing posture, which is the Supreme Overlord's default for the work rather than any task's answer; `AGENTS.md` section 7 owns how each task's concrete mode and yolo are resolved at intake and passed explicitly to the brief, the spawn, and any promotion.
...
`no-mistakes-prod-only` is the default for a newly added or created remote-backed project when the Supreme Overlord specifies nothing, and a project with no remote defaults to `local-only`.
State that resolved default while confirming the source, local name, and posture instead of asking the Supreme Overlord to choose from scratch, and record a flat mode instead whenever they ask for one.
...
Default it off for every project and every posture, and enable it only on the Supreme Overlord's explicit instruction.
`AGENTS.md` section 7 owns the complete authority boundary and exceptions when it is on.

## Add or clone an existing project

Confirm the source URL, local project name, delivery posture, and autonomy posture, stating the resolved default for each rather than asking the Supreme Overlord to invent one.
...
The Supreme Overlord's request to create that local project authorizes this local initialization, but it does not authorize an unmentioned remote repository.
...
First obtain the Supreme Overlord's explicit removal decision, then inspect the current digest and authoritative repositories for in-flight or queued work, registered secondmate clones, linked worktrees, dirty files, unpushed commits, and any other unlanded work.
If any dependency or unlanded work exists, stop and report it before changing anything.
Never issue a raw removal command from CFO.
Once that preflight confirms none of the above and the Supreme Overlord's approval is concrete, AGENTS.md hard rule 1's overlord-approved project operation exception authorizes CFO to remove the clone directly and update its registry entry to match.
