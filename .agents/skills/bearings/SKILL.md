...
name: bearings
description: >-
  Generate a "pick up where I left off" fleet digest from CFO's live fleet state.
  Use when the Supreme Overlord invokes /bearings or asks for a bearings report, morning brief, status report, catch-up, "where did I leave off", or "what's in the works".
...
Generate a complete current snapshot from the fleet's current state, so the Supreme Overlord can resume in one read after a break, a night, or a context reset.
...
- When the Supreme Overlord asks to include PRs, pass the snapshot command's live-PR opt-in.
...
1. **Gather live fleet state with one deterministic command.**
   ...
   Keep the default local-only read unless the Supreme Overlord asks to include PRs.
   ...
   Render it under Charted Next with the related `omitted` disclosure, never invent an Underway row from backlog-only state, and never move it into Supreme Overlord's Call.

2. **Compose the four-section chat digest from the fresh snapshot.**
   The gather step is deterministic; your judgment is scoped to ranking the command's facts by what matters right now and writing scannable overlord-facing prose.
...
3. **In explicit file mode only, compose and replace the detailed report file.**
   ...
   - **Title** - `# Bearings - <day> <YYYY-MM-DD>` (use "Morning status" only when the Supreme Overlord specifically asks for a morning brief), followed by two or three sentences framing where things stand.
   - **Supreme Overlord's Call** - every open decision summarized with its options from the structured decision record, plus each PR ready to merge and each needed credential or login, every PR with the full `https://...` URL, never a bare `#number`.
...
1. **Supreme Overlord's Call** - ONLY items that need the Supreme Overlord's own action now: a decision to make, a PR to approve or merge, a credential or login to provide, or a blocker only the Supreme Overlord can clear.
...
4. **Charted Next** - queued or gated work waiting on the fleet or a date, plus action-free fleet-integrity warnings, never on the Supreme Overlord.
...
- The four buckets are mutually exclusive, so every item is forced into exactly one: needs-your-action is Supreme Overlord's Call, done is Recently Landed, self-progressing is Underway, and not-yet-started work or an action-free fleet-integrity warning is Charted Next.
- The strict boundary keeps action-free items OUT of Supreme Overlord's Call: a working or validating task, a queued item blocked on another task or a date, landed work, a completed scout's report pointer, a declared `paused:` external wait, and a bare recorded PR with no merge-ready signal each belong to one of the other three sections, never Supreme Overlord's Call.
- A secondmate's own row appears Underway only for `active_child_work`; `externally_held` belongs in Charted Next, and `unknown` belongs there as an unavailable-state gate unless its reason requires the Supreme Overlord's action.
...
- Include the required direct address to the Supreme Overlord inside one item or empty-state sentence.
...
- The optional file-mode report is a private, overlord-facing internal artifact that lives in gitignored `data/`, so unlike normal Supreme Overlord chat it MAY reference task ids, PR URLs, and repo names.
- The Supreme Overlord works with those directly and needs them to resume; keep the report organized and scannable, not a raw dump.
