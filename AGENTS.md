...
| `cfo pipeline config-drift \| config-apply \| run <id> --intent <text> \| respond <id> --action <fix\|approve> [--findings <ids>] [--instructions <text>]` | Drive a gated task under its frozen policy: report or apply the owned shared no-mistakes settings in an explicit idle window, start a bounded gate run, and answer one parked review with an explicit fix or a no-op approval. Never auto-approves and never skips a step; exit 3 means unresolved work needs your decision. Full contract in [docs/pipeline.md](docs/pipeline.md) |
| `cfo send <target> [--no-auto-submit] <text>` | Type a steer to a goblin; after a failed Enter submit, verifies the text is parked in the composer and resubmits with the harness-specific key (pi/claude/codex: Enter, kimi: ctrl+s) — `--no-auto-submit` opts out |
...
| `cfo reap [--dry-run] [--apply] [--force <pid\|task-id>]` | Find what nothing else notices, and with `--apply` retire it: a harness process whose pane is gone (unsupervised, invisible to `herdr agent list`, still spending tokens), a dev server left running in a finished goblin's worktree, an orphaned worktree, task record or status log. Reporting is the default. `--apply` never kills a process still burning processor time, never removes a worktree with uncommitted or unpushed work, and never reaps a task that has not finished; `--force` names one pid or task id to override the first two of those, never the work gate. The watcher sweeps on its own timer and the session-start digest prints the result, so an orphan reaches you without your asking |
...
- `no-mistakes` — the goblin runs the pipeline through `cfo pipeline run`/`respond` under its frozen policy, so an exhausted review budget comes back as unresolved work needing your decision rather than an automatic approval; you relay the PR URL and wait for merge authority, then `cfo pr merge`.
...
- **Every drain ends with a `WAKE_ACK_REQUIRED` line. Run it.** An unacked record resurfaces on the next drain and makes a handled goblin look unhandled. That line is refused while an unanswered `--blocked`/`--failed` notify sits at or below its sequence: drain names the waiting goblin and retires nothing. The refusal is the protection working, not an error - a record acked unread is a record nobody will ever read. Answer the goblin with `cfo send <id> "..."`, then re-run the same command with `--ack-blocking` to retire the question you have now read.
