...
- A persistent secondmate keeps supervising its own children without the captain or away mode via self-supervise mode: `state/.self-supervise`, owned by `bin/fm-afk-launch.sh` (`docs/self-supervision.md`). It is started with NO model action - automatically on child dispatch (`bin/fm-spawn.sh`) and reconciled at session start (the secondmate-liveness sweep); the daemon self-exits when the queue empties. There a marked (`FM_INJECT_MARK`) injection is an internal supervise-resume the secondmate drains and advances on its own turn - it never changes approval authority, and unlike `.afk` it has no captain-return exit.
