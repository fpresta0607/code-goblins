...
| `cfo notify <id> --done --pr <url> \| --blocked "<question>" \| --failed "<reason>"` | A goblin reports its outcome (PR URL, blocked question, or failure reason) straight into the wake queue, waking the CFO with the real payload instead of the watcher guessing from pane text. A blocked question that names its choices as `<question> options: a \| b \| c` is rendered by `cfo drain` as a decision with those options listed |
