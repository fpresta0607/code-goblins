...
Answer a goblin's question yourself with `cfo answer <wake-seq> --option <choice> [--note "<text>"]` rather than `cfo send` plus `--ack-blocking`: the goblin receives the same `CFO:` line, the notify retires through an ordinary drain, and the board shows a check on the option you chose with "Answered by the CFO" instead of a question closed without an answer.
