...
| --- | --- |
| `cfo install [--projects-root <dir>] [--uninstall]` | Wire a CFO home into the machine so a Claude Code session opened in any repository is supervised: `CFO_HOME` and PATH at user scope, and the CFO hooks merged into the user's `~/.claude/settings.json` (their own hooks are kept, the file is backed up first). Run from a checkout, the home is that checkout. Run anywhere else, it is `%LOCALAPPDATA%\CodeGoblins`, set up from the binary itself: this contract and the docs it links to, the skills (under `.agents\skills` and `.claude\skills`), the default `config/pipeline.json` and `data/routing.json` (written only where missing, so a tuned policy is kept), the binary as `cfo.exe` and `goblins.exe`, and a `.cfo-home` marker that makes the folder a primary home without a git checkout; a re-run brings the contract, skills and binary up to date and removes the files an older binary wrote that this one no longer ships (the marker lists them; a file you added is never touched). Either way it refuses while `CFO_HOME` names another home in use: run it from that home to keep it, or run `goblins uninstall` there first to move. `--projects-root <dir>` records the folder that holds your checkouts as `CFO_PROJECTS_ROOT`, beside `CFO_HOME` at user scope and never in this repository, which is what lets `--project` take a bare name; a plain re-install keeps whatever is recorded. Idempotent; `--uninstall` (or `cfo uninstall`, `goblins uninstall`) reverses the wiring, the projects root and the board's native hooks from `cfo hooks install` included, and keeps a per-user home's files. `cfo doctor` reports, this repairs |
| `cfo doctor` | Check git, gh, claude, herdr, codex, pi, kimi, tasks-axi, quota-axi, no-mistakes, gh-axi, chrome-devtools-axi and print install hints; check `lavish-axi` against its `0.1.71` floor as a presentation-only dependency, reported as `PRESENTATION_UNAVAILABLE` when missing or older and never counted against the health verdict; report a missing `winget` as `INSTALLER_UNAVAILABLE` with its fix (App Installer from the Microsoft Store), never counted against the health verdict because only `install.ps1` runs it; probe each installed harness (`--version` under a short timeout) and report ok/broken; print the validation-invocation timing table from `~/.no-mistakes/state.sqlite` when present, split by harness, recorded model, role, step, and outcome so failure latency is never read as coding speed, and labeled implementation-unmeasured because that database observes validation agents only (skipped with a note when absent or locked); print the standing switch rules and the execution lane table from `data/routing.json`, so which model each kind of work gets is visible without opening the file; print the projects root, or the `cfo install --projects-root <dir>` that records one when it is unset, which never counts against the health verdict because every command still takes a path |
...
| `cfo register` | Make this session the primary CFO the board delivers questions, answers, reviews and terminal input to. The SessionStart hooks do it on their own; run it by hand when the board reports the registration stale or missing. It refuses unless Herdr shows your own harness in the foreground of your pane, or your native terminal's live `cfo host` runs it, and that harness holds the home |
| `cfo notify <id> --done --pr <url> \| --blocked "<question>" \| --failed "<reason>" \| --working "<what>" \| --waiting-on <task-id\|overlord\|ci\|deploy> "<why>" [--lavish <html-file>]` | A goblin reports its outcome (PR URL, blocked question, or failure reason) straight into the wake queue, waking the CFO with the real payload instead of the watcher guessing from pane text. **`--done` is per pull request, not per task: it requires `--pr` and records the literal line `done: PR <url>`, and a task can ship several under one id, which is the ordinary shape for a landing task.** So a terminal verb in a status log establishes that a pull request finished and nothing whatever about whether the goblin is still working; anything that reads it as "the goblin is gone" is wrong, and the more destructive the thing it decides, the more wrong. Live evidence answers that question: a pane holding an agent working in the goblin's worktree. A blocked question that names its choices as `<question> options: a \| b \| c` is rendered by `cfo drain` as a decision with those options listed. `--working` and `--waiting-on` are status for the board: they wake nobody and replace a stale blocked or failed reading, except that waiting on the Overlord wakes the CFO and puts an item in the Command Center; a wait on another task clears itself when that task reports done, and the CFO releases any wait with `cfo notify <id> --working "<what>"`. A wait on the Overlord that needs his answer on a Lavish page names it with `--lavish <html-file>`: the notify opens the page without a browser and refuses, recording nothing, when it cannot; the page's link goes into the wait's line and onto its Command Center item, and the supervisor polls the page, so his feedback there reaches the CFO as a `review` wake with the whole reply saved under `state/reviews/feedback/`, and the item closes. A goblin never runs `lavish-axi poll` itself: that poll takes his feedback where only the goblin sees it. An actual question still uses `--blocked` with options. |
...
- The goblin receives the token-authenticated subset of the project's `.mcp.json`: stdio servers and HTTP servers with a bearer token qualify; OAuth-only connectors are withheld and named on the `spawned` line, because a goblin can never complete their browser flow.
  ...
  A kept URL server with no `type` gets the one Claude needs (`sse` for an `/sse` endpoint, otherwise `http`), and an explicit `type` is kept; for any kept URL server without its own `Authorization` header, typed or not, a `bearerTokenEnvVar`, which Claude does not read, becomes its `Authorization` header as an unexpanded `${VARIABLE}` reference.
...
- **A stale wake means the goblin's own evidence stopped, not that its turn is old.** Before the monitor raises `busy_turn_over_age`, `awaiting_answer` or `unchanged_idle`, it reads the goblin's harness transcript (Claude, Codex and Pi, located by the session Herdr reports) and the processes its harness started after launching (tool commands, background jobs, monitor loops; its MCP servers start with it and never count). A turn whose transcript was written, or whose own processes used at least 5% of a processor, within the busy budget (one hour) is progressing however old it is, and that processor use is judged reading by reading over spans of at least a minute, while a lack of it is concluded only after a full stall interval (ten minutes) of readings; a goblin that ended its turn with its own job still running and moving is waiting on that job, reads busy, and is asked about only when the job is gone or has shown no progress for the budget, when the wake names what is still running. Once raised, the same goblin is raised again only after its evidence has moved and stopped again. Evidence that cannot be read is no evidence: the wake fires as it did before and says so.
...
- When several options, trade-offs, a structured report, a plan, or a comparison need the Supreme Overlord's eyes, load the `lavish` skill (`.agents/skills/lavish/`), write the page, and publish it with `cfo review --id <stable-id> --title "<what to look at>" --lavish <html-file>`: the supervisor polls the page and his feedback reaches you as a `review` wake, so you never hold your turn on `lavish-axi poll`.
- `lavish-axi` is presentation-only. Only pages named by their files (`cfo notify --lavish` and `cfo review --lavish <html-file>`) and the supervisor's polls of them use it, so when it is missing or below its floor you say visual review is unavailable once, deliver the same content as text, and keep working; a page wait or a page review is refused and asks in text instead. Nothing else waits on it.
...
For something the Overlord should look at without blocking the goblin, brief it to run `cfo review --id <stable-id> --task <its id> --title "<what to look at>" [--image <path>]... [--lavish <url|html-file>]` from its own pane: the item stays in the Command Center until he clears it, the goblin withdraws it with `--withdraw "<reason>"`, or the goblin finishes or is cleaned up, and its images are copied so they outlive the worktree. Clear a stale item yourself, such as one a retired goblin left, with `cfo review --clear <id> --reason "<why>"`; the clear is audited in `state/reviews.audit`. A page named by its HTML file is polled by the supervisor, so his feedback on it reaches you as a `review` wake and the item closes. His board answer on a goblin's item or wait goes to the goblin and also to you, as a `review` notice that asks nothing.
To hand the Overlord a finished file, such as a PDF report, run `cfo deliver --id <stable-id> --title "<what it is>" --file <path> [--url <link>]` (a goblin adds `--task <its id>` and delivers only from its own worktree, task scratch or data directory): the Command Center shows it as a document card with Open and Download, and it leaves the queue once he opens or downloads it.
...
Answer a goblin's question yourself with `cfo answer <wake-seq> --option <choice> [--note "<text>"]` rather than `cfo send` plus `--ack-blocking`: the goblin receives the same `CFO:` line, the notify retires through an ordinary drain, and the board shows a check on the option you chose with "Answered by the CFO" instead of "The CFO already handled this question."
Other worker alerts stay in the CFO wake queue until the CFO deliberately escalates a real user decision.
When you need the Overlord to run a command you cannot run yourself (a classifier refusal, a step that needs administrator rights, or credentials only he holds), create a run item instead of pasting the command into chat: write the exact command to a file and run `cfo run-request --id <stable-id> --title "<why>" --shell powershell|pwsh|bash [--admin] [--cwd <dir>] --command-file <path>` from verified primary context. He runs it with one click from the Command Center, in exactly that shell and through Windows UAC for `--admin`, and its exit code and output come back to you as his answer. Never put a secret in a run command or request one that prints a secret: the output is stored on the board.
