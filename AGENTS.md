...
| `cfo pipeline config-drift \| config-apply \| migrate <id> \| run <id> --intent <text> \| respond <id> --action <fix\|approve> [--findings <ids>] [--instructions <text>]` | Drive a gated task under its frozen policy: report or apply the owned shared no-mistakes settings in an explicit idle window, explicitly migrate an idle v1 task snapshot while preserving its class and repair cap, start a bounded gate run, and answer one parked review with an explicit fix or a no-op approval. Never auto-approves and never skips a step; exit 3 means unresolved work needs your decision. Full contract in [docs/pipeline.md](docs/pipeline.md) |
