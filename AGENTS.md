...
| `cfo send <target> <text>` | Steer a goblin: the message goes through Herdr's native agent channel and the send reports success only once Herdr's own agent counters show the agent accepted it. It is submitted once, so an unconfirmed send is checked with `cfo peek` and repeated deliberately rather than automatically. A target Herdr reports no agent for - an explicit `<session>:<pane-id>` sitting at a shell prompt - gets the text typed and submitted with Enter instead |
