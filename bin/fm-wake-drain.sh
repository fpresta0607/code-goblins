...
collect_inactive_outcomes() { # <sequence>
...
if [ -n "$ACK_THROUGH" ]; then
  ...
  INACTIVE_OUTCOME_FINGERPRINTS=$(collect_inactive_outcomes "$ACK_THROUGH") || exit 1
  fm_lock_release "$FM_WAKE_QUEUE_LOCK"
  DRAIN_LOCK_HELD=false
  if ! acknowledge_inactive_outcomes "$INACTIVE_OUTCOME_FINGERPRINTS"; then
