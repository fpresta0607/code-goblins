...
A tab whose loaded build differs from the one served, such as one left open across an install, reloads itself while it is hidden and the Overlord is not mid-answer (the Command Center closed and no text field holding text), and otherwise shows one line, The board was updated, with Reload; a page loaded without a build, such as from the dev server, never acts.
