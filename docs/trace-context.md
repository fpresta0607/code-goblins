...
CFO can propagate a W3C [`traceparent`](https://www.w3.org/TR/trace-context/) to every agent it spawns so an external observer can identify each task as exactly one trace and correlate everything that task runs under that one identity.
...
CFO's durable operational artifacts already let a downstream observer derive logical task identity and lifecycle.
...
When enabled, for each spawn CFO resolves one W3C `traceparent` carrier for the task - minted as a fresh root on the task's first spawn and reused verbatim from the meta on relaunch - and:
...
`TRACEPARENT` as an environment variable is a CFO convention carrying a W3C-formatted value: W3C Trace Context standardizes the `traceparent` HTTP header, not an env var, and OpenTelemetry SDKs do not read it from the environment automatically, so a downstream observer must explicitly read this env value or the `traceparent=` meta field.
...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns across `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `muse`, plus Secondmate spawns across that same set except the deliberately goblin-only `muse` adapter.
...
CFO does not monitor setting drift, detect mismatches, refuse launches, or automatically stop or restart any home.
...
- **A recorded carrier keeps its flags verbatim.**
  ...
  CFO chooses the flag only when it mints a *root*, which is the only way a new carrier is created.
- **Cost and privacy consequence.**
  `01` records a sampling *decision*, and a conforming downstream parent-based sampler will honor it - but it does not by itself guarantee that any collector stores a span, and CFO emits no spans of its own; it only sets the flag on the carrier.
  ...
  An operator who wants unsampled roots or head-sampling owns that downstream or via a later, explicitly-scoped option; CFO does not embed a sampler.
...
- **What is and is not exposed.**
  A CFO-*minted* root uses a random id and reads no prompt, path, task prose, credential, or arbitrary environment key, so CFO never *originates* sensitive data in the carrier.
  Every carrier CFO injects is either such a mint or the same task's previously recorded carrier reused verbatim; ambient `TRACEPARENT` is never read, so no caller-controlled bytes enter a new carrier.
...
- **Fail-independent.**
  ...
  If the pre-launch carrier export fails, CFO omits the `traceparent=` metadata claim and still launches the task.
  If the backend reports that failed trace input could not be cleared, CFO refuses to append the launch command rather than risk launching with an unknown partial carrier.
  If recording the carrier fails after export, CFO unsets `TRACEPARENT` in the launch command and still launches the task, so the child never receives an identity absent from its metadata.
- **Metadata-only.**
...
CFO learns nothing about OpenTelemetry, any exporter, collector, storage, or UI.
