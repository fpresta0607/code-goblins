...
The launch sequence must set `GOTMPDIR` to a per-task directory outside the fleet checkout (`<machine temp>\cfo-gotmp\<id>`, created by spawn and removed by cleanup), send the launch line literally, send Enter as a separate key, and confirm a working agent before reporting success.
