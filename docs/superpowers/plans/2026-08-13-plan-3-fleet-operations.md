...
> **Superseded — native agent launch and cfo workspace.** Spawn now starts natively launchable harnesses as named Herdr agents via `herdr agent start` (harness args after `--`) and delivers their brief via `herdr agent prompt`; a harness installed as an npm `.cmd` shim launches as a typed command in the prepared pane shell instead, because Herdr's Windows agent start cannot run those shims. Spawn confirms each harness's declared trust dialog with adapter-specific keys, and adopts or creates the flat workspace labeled `cfo` instead of `code-goblins`. The affected sections are Task 3 (`EnsureContainer` workspace label), Task 5 (`Launch` shape and `PowerShellLine`), and Task 7 (launch sequence); their authoritative owners are the `internal/herdr`, `internal/harness`, and `internal/spawn` doc comments plus `README.md` and `AGENTS.md`. Those task texts below remain historical and are not re-synced.
...
> **Superseded - native text sends.** The Task 8 text-send contract below (type once with `pane send-text`, settle, retry Enter, and confirm from the Herdr agent or composer state) was replaced: `cfo send` now submits through `herdr agent prompt` once and confirms from Herdr's own monotonic agent counters, and composer inspection is gone along with the `--no-auto-submit` flag that selected between its resubmission strategies. Only an explicitly addressed `<session>:<pane-id>` pane holding no registered agent is still typed into, and that delivery is always reported unconfirmed. The authoritative owners are the `internal/fleet` and `internal/herdr` doc comments plus the `cfo send` rows in `AGENTS.md`; the Task 8 task text below remains historical and is not re-synced.
...
The launch sequence must set `GOTMPDIR` to a per-task directory outside the fleet checkout (`<user cache dir>\cfo\gotmp\<fleet>\<id>`, where `<fleet>` is a short hash of the fleet's state directory so two homes on one machine can never share one; created by spawn, reused by switch, removed by cleanup; the user cache directory rather than `%TEMP%`, which Windows prunes on its own while a task is live for days), send the launch line literally, send Enter as a separate key, and confirm a working agent before reporting success.
