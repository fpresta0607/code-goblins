...
The launch sequence must set `GOTMPDIR` to a per-task directory outside the fleet checkout (`<user cache dir>\cfo\gotmp\<id>`, created by spawn and removed by cleanup; the user cache directory rather than `%TEMP%`, which Windows prunes on its own while a task is live for days), send the launch line literally, send Enter as a separate key, and confirm a working agent before reporting success.
