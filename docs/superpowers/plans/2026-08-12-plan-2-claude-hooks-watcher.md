...
> **Superseded — the presentation fold is deleted.** `wake.Deduped` no longer exists and `wake.Render` prints EVERY unacknowledged record. The fold collapsed records last-write-wins per `(kind,key)` while the ack-through sequence came from the raw maximum, so the printed ack line retired records the listing had declined to show; `blockingAtOrBelow` read the same folded view and was blind to the blocked notifies it exists to protect. Two real escalations were lost that way. Render now derives the ack sequence from the rows it actually wrote, via `ackSequence`, which refuses when those rows do not account for every record given. The authoritative owner is `internal/wake/wake.go` (`Render`, `ackSequence`) with the guard in `cmd/cfo/drain.go` (`blockingAtOrBelow`). Every mention of the fold below has been corrected to specify current behaviour, with one deliberate exception: the `git commit -m` string in Task 5's commit step still reads `dedup`, because it is the historical command that was actually run. Do not restore the fold as an optimisation.
...
internal/wake/wake.go              MODIFY: Kind whitelist, Key field, unfolded presentation
...
- Produces:
  ...
  - `cfo drain [--ack-through <seq>] [--ack-blocking] [--recovery-generation <gen>]`: with no flags, prints every unacknowledged pending record and any pending episode, then the exact ack command line; with flags, performs the acks. `--ack-through` alone acks sequences and never touches the episode; `--ack-blocking` is range-scoped, not per-record: it retires every record at or below that sequence, including unanswered blocked or failed notifies from other goblins; `--recovery-generation` alone acks the episode only when the queue is already empty. Exit 0 in every non-error case, including generation mismatch (which prints `recovery generation moved, re-run: cfo drain`). A write error on the output stream is the one nonzero exit `runDrain` produces, and Task 12's extraction of the renderer into `wake.Render` preserves it as that function's error return.
...
The three fold tests this step originally specified (`TestDedupedLastWriteWinsPerKindKey`, `TestDedupedCollapsesHeartbeats`, `TestDedupedKeepsBothBucketsAcrossCycles`) are gone with the fold itself. What replaced them asserts the opposite property: every appended record survives into the listing, and the ack sequence never outruns the rows printed.
...
`wake.go`: add the whitelist check at the top of Append (`var kinds = map[string]bool{...}`); add Key to Record and to the append path. No presentation fold: the renderer prints every record it is given, in append order.
...
- Produces:
  ...
  - `watch.Run(cfg Config) (reason string, err error)`: acquires the singleton, then defers `lock.ReleaseNamed(cfg.Home.State, ".watch.lock")` FIRST and `cfg.Cleanup()` (when non-nil) SECOND, so Go's LIFO ordering runs Cleanup before the release. The singleton comes from `lock.AcquireNamedOwner(cfg.Home.State, ".watch.lock", os.Getpid(), "watch")`. Then it loops: touch `state/.last-watcher-beat`; scan signals; on changed status files, sleep SignalGrace, rescan, append one wake record PER CHANGED FILE (`kind signal`, `key <that file's own basename>`, `detail "signal:<space-joined relative paths>"`, the same detail on every record of the cycle), call `CommitSignatures` for exactly those changes once every Append has returned nil, and return the shared detail as the reason; when the heartbeat is due, append `(heartbeat, heartbeat, "heartbeat")` and return `"heartbeat"`.
    One record per changed file is upstream's shape (`bin/fm-watch.sh:977`) and it is what gives each changed file its own row in the listing. It was originally justified as what made the `(kind,key)` fold correct - a single record keyed on an arbitrary member of the changed set let a later cycle overwrite an earlier cycle's mention of a file whose signature had already advanced, so that file's completion was never presented and was then acked away. That is the exact failure for which the fold has since been deleted; per-file keying stands on its own and stays.
    Between checks it waits via `WaitEvent(Poll)` when non-nil else `Sleep(Poll)`.
    On a CLOSE path (an actionable signal or heartbeat reason), `PublishEpisode(cfg.Home.State)` is called BEFORE releasing the singleton (upstream's watcher-down marker); when the singleton was lost to a successor (`lock.HeldByNamed(cfg.Home.State, ".watch.lock", os.Getpid())` no longer names us), return `("", nil)` without publishing.
    An ERROR return publishes nothing. An earlier revision of this plan said every path except lock-lost publishes; that was wrong. A transient `os.Stat` or read failure must not churn the recovery generation that `AckEpisode` keys on, and an error return that appended no record would otherwise leave an episode with an empty queue. The consequence is that an error exit is silent to the fleet, so Task 11 owns publishing the watcher-down episode on the arm side when `Run` returns an error, and must account for the one error path that can leave queued records with no episode (an Append succeeded, then CommitSignatures or a beacon write failed).
  ...
  - `watch.CommitSignatures(stateDir string, changes []Change) error`: writes those signatures, and Run calls it only after every wake Append for the cycle has returned nil. Detection and commitment are separate so a watcher killed inside the SignalGrace window, or any error return before Append, re-reports the same signal on the next start instead of swallowing it permanently. Signatures still persist across watcher restarts (that is the point: signals landing while no watcher runs are caught on the next start), and a crash between Append and CommitSignatures costs a duplicate wake, now shown as its own row in the listing, rather than a lost one. Showing the duplicate is the point: the fold that used to hide it hid unread escalations by the same rule. Because ScanSignals commits nothing, the post-grace rescan already returns the union of both scans and no separate coalescing step is needed.
  - `watch.Sanitize(name string) string`: maps every character outside `[A-Za-z0-9_-]` to `_` (covers `:` `/` `\` `.` which are illegal or ambiguous in NTFS filenames). Never widen it to preserve dots, or `.seen-a.status` becomes indistinguishable from a hidden state file.
  - The mapping is lossy, so the signature filename is `.seen-<sanitized>-<8 lowercase hex of FNV-1a-32 over the raw basename>` (stdlib `hash/fnv`). The hash suffix is load-bearing, not decoration. An earlier revision of this plan claimed a collision merely merged two signatures and cost a duplicate wake that the presentation fold would hide. That was wrong and is recorded here so nobody restores it. Two ids that sanitize to the same name would share one signature file, so each scan reports the other one as changed, forever: every cycle closes on a signal, appends a record, and publishes an episode, which under Task 11's arm loop is an unbounded rewake of the CFO and unbounded recovery-generation churn. Folding duplicate records - which the renderer no longer does at all - would do nothing about a scan that re-triggers endlessly.
...
- One reusable 4KB buffer per waiter.
  Wait decodes the FILE_NOTIFY_INFORMATION records packed into that buffer and returns true only when at least one record names a file ending in .status or .turn-ended, matched case-insensitively.
  FileNameLength is a byte count over UTF-16, and NextEntryOffset 0 marks the last record.
  This is an allowlist of the two suffixes ScanSignals already acts on, not a denylist of today's known bookkeeping files, so it stays correct when a later task adds a new bookkeeping filename instead of rotting the way a denylist of .last-watcher-beat, .seen-*, and .watch.lock would.
  The filter exists because watch.Run touches state/.last-watcher-beat at the top of every iteration, on this same directory, under this same mask.
  Without the filter, the kernel buffers that self-inflicted touch on the handle and hands it back on the very next read, so Wait returns true in 0ms starting from the second iteration and the loop free-runs at full CPU for as long as the host process lives.
  A zero-byte or buffer-overflow completion (ERROR_NOTIFY_ENUM_DIR) still returns true, since the content is lost and a change definitely occurred.
  When a completed read's records are all filtered out, Wait reissues the read for whatever remains of a deadline computed once at entry from the caller's timeout, rather than returning false immediately, so a real status write landing later in the same interval is still caught promptly instead of costing up to a full Poll of latency.
...
**Interfaces:**
- Consumes: `lock.AcquireOwner`/`HeldBy`, `proc.FindAncestor`, `wake.Pending`/`ReadEpisode`/`Render` (Render owns the whole presentation, so Compose passes raw pending records and decides nothing about them), `state.ReadMeta`/`TailStatus`, `fsx.ReadLines`, `home`, `claudehook` env.
- Produces:
  - `digest.Compose(h home.Home, ownerPID int, session string, w io.Writer) error`: writes the full digest in this exact section order, each section beginning with a `== <NAME> ==` header line:
    ...
    ●━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ```
    2. `== WAKE QUEUE ==`: `wake.Render` of the RAW pending records and the episode; read-only, never acks. Render takes raw pending records, prints every one of them, and derives the ack-through sequence from the rows it wrote via `ackSequence`, so neither call site has to decide what to pass and the ack line can never cover a record the listing withheld. A write error from Render propagates as the Compose-level failure named in the exit-code paragraph below, and `runDrain` maps the same non-nil return to a nonzero exit.
    3. `== SUPERVISION OPERATING INSTRUCTIONS ==`: fixed text block stating: the Stop-owned auto-arm owns watcher continuity; never run `cfo watch` from the agent shell; wakes arrive as rewake turns; every drain presentation ends with a WAKE_ACK_REQUIRED command that must be run after handling, and that command is refused while an unanswered blocked/failed notify sits in range, so the completion path is to answer the goblin and re-run it with `--ack-blocking`; supervision is needed whenever tasks are in flight.
