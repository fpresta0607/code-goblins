...
> **Superseded — the presentation fold is deleted.** `wake.Deduped` no longer exists and `wake.Render` prints EVERY unacknowledged record. The fold collapsed records last-write-wins per `(kind,key)` while the ack-through sequence came from the raw maximum, so the printed ack line retired records the listing had declined to show; `blockingAtOrBelow` read the same folded view and was blind to the blocked notifies it exists to protect. Two real escalations were lost that way. Render now derives the ack sequence from the rows it actually wrote, via `ackSequence`, which refuses when those rows do not account for every record given. The authoritative owner is `internal/wake/wake.go` (`Render`, `ackSequence`) with the guard in `cmd/cfo/drain.go` (`blockingAtOrBelow`). The task texts below are corrected where they specified the fold; do not restore it as an optimisation.
