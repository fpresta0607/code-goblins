...
- A respawn with exact owned metadata reuses a dead or agent-free recorded endpoint, while a reused task id with no metadata adopts one structurally restricted preserved workspace, records exact replacement endpoints after partial failures, converges stale husk duplicates on retry, and refuses live, mismatched-parent, unrelated-tab, or otherwise ambiguous shapes.
...
- Opted-in child-workspace tasks acquire a durable Treehouse lease whose machine-readable pool identity is recorded in both task metadata and a separate exact worktree-to-lease binding.
- Pre-identity metadata is migrated only when that durable binding names its recorded worktree and still exactly matches Treehouse's live lease identity; a missing binding, another home's binding, and every returned, reassigned, owner-only, or ambiguous pool entry retain recovery metadata and fail safely.
...
| Send + submit atomically | `herdr pane run <pane> <command>` | Runs and submits a command in one call; used for the fixed spawn-time Treehouse acquire or lease command and the `GOTMPDIR` export exactly where tmux used one `send-keys ... Enter` call. |
