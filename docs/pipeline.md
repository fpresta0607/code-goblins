...
A repository-scoped durable binding records the exact task, task worktree, native run worktree, launch identity, and CFO executable identity independently of the daemon environment, so a `cfo-v1` invocation fails closed before any database lookup when both task-local evidence files are absent or invalid, while an unmanaged invocation does not require `sqlite3` on `PATH`.
