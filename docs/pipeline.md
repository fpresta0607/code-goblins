...
The managed native command carries a process-scoped nonce and validation-generation seal, so a `cfo-v1` invocation fails closed before any database lookup when both task-local evidence files are absent or invalid, while an unmanaged invocation does not require `sqlite3` on `PATH`.
