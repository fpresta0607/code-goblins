...
Immediately before managed delegation, the guard requires the current CFO executable path and digest to match the contract, confirms ChatGPT authentication, forces the OpenAI provider, ChatGPT login method, and canonical ChatGPT Codex endpoint, disables OpenTelemetry exporters and prompt logging, and removes every known harness billing credential, alternate OpenAI base URL, proxy setting, and OpenTelemetry environment override from the inherited environment case-insensitively.
Marker invocations outside a CFO-managed run retain their original provider arguments and environment.
Task-scoped durable bindings record each exact task, task worktree, native run worktree, launch identity, and CFO executable identity independently of the daemon environment, so concurrent tasks in one repository remain isolated, a `cfo-v1` invocation fails closed before any database lookup when both task-local evidence files are absent or invalid, and an unmanaged invocation does not require `sqlite3` on `PATH`.
...
The initial head must equal the submitted head, while a later head requires an ordered durable chain from the submitted head through the completed rebase output and each subsequent review or fixer range before the guard rechecks the shared configuration, origin default branch, tracking ref, both repository configuration blobs, and Codex primary.
...
When the first managed role is a rebase fixer, the driver verifies its recorded OpenAI provider and Codex gpt-5.6-sol model, retains the contract as authorized, and waits for review provenance without accepting a mismatched transition range.
After the first managed review returns, the driver additionally requires the trusted configuration SHA and SHA-256 of the persisted review global configuration before atomically promoting the contract to verified.
