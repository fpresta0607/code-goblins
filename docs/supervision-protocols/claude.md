...
1. Drain first with `bin/fm-wake-drain.sh`. After handling every emitted wake, run the exact `--ack-through` command printed as `WAKE_ACK_REQUIRED`; until then the wake remains durable for idempotent re-handling after interruption.
