...
4. On the one `Stop hook feedback` automatic-mechanism failure notice (`CFO watcher auto-arm FAILED ...`), drain, inspect the automatic mechanism failure, and do not turn the notice into a repeating manual-arm loop.
