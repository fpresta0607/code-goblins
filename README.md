...
- **Explicit project modes** - each project ships via `no-mistakes`, `direct-PR`, or `local-only`, or the conditional `no-mistakes-prod-only` policy that runs the pipeline for product-facing work and opens a direct PR for internal-only work and is what a new remote-backed project starts as; an optional `+yolo` autonomy flag is orthogonal and off by default, and a single task can carry its own recorded delivery resolution.
